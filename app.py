...
# 2. FONCTION DE RÉPARATION DE CLÉ (Méthode blindée)
def get_gspread_client():
    # Ta clé brute telle qu'elle sort du JSON
    ...
    # ON NETTOIE : remplace les doubles slashs et les n par de vrais sauts de ligne
...
for index, row in df.iterrows():
    ...
    with c1:
        # On vérifie si possede est 1 ou 0 (parfois gspread lit des strings)
        is_owned = str(row['possede']) == "1"
        if st.button("✅" if is_owned else "⬜", key=f"btn_{index}"):
            new_val = 0 if is_owned else 1
            # Colonne E (5) pour 'possede'
            worksheet.update_cell(index + 2, 5, new_val)
            st.rerun()
    with c2:
        st.write(f"**{row['nom']}** | Base: {row['base']} | Page: {row['page']}")
